...
if ls ../install/lib/libredis++.a 1>/dev/null 2>&1; then
    ...
    mkdir compile
    cd compile
    $CMAKE -DCMAKE_BUILD_TYPE=Release -DREDIS_PLUS_PLUS_BUILD_TEST=OFF -DREDIS_PLUS_PLUS_BUILD_SHARED=OFF -DCMAKE_PREFIX_PATH="$(pwd)/../../../install/$MPI_ROOT/lib/" -DCMAKE_INSTALL_PREFIX="$(pwd)/../../../install/" -DCMAKE_CXX_STANDARD=17 ..
